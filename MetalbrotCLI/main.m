...
//  MetalbrotCLI
//
//  Created by Jocelyn Manger on 1/31/26.
//
    ...
    @autoreleasepool {
        ...
        for (NSInteger i = 1; i < (NSInteger)args.count; i++) {
            ...
            if ([arg isEqualToString:@"--help"]) {
                ...
                NSString *value = nextArg(args, &i);
                if (!value) { printUsage(); return 1; }
                scheme = (uint32_t)value.intValue;
        ...
        //[iterTexture isFramebufferOnly:YES];
        ...
        colorTexDesc.storageMode = MTLStorageModePrivate;
        ...
        // First pass: write normalized iteration values into the offscreen texture.
        [iterEncoder setRenderPipelineState:iterPipeline];
        [iterEncoder setVertexBuffer:vertexBuffer offset:0 atIndex:0];
        [iterEncoder setFragmentBytes:&params length:sizeof(params) atIndex:0];
        ...
        // Second pass: shade with the palette using iteration texture.
        ...
        [colorEncoder setFragmentBytes:&params length:sizeof(params) atIndex:0];
        ...
        // Blit pass copies the color texture into a CPU-visible buffer.
     ...
     destinationBytesPerImage:bufferLength];
        [blit optimizeContentsForCPUAccess:colorTexture];
        [blit endEncoding];
